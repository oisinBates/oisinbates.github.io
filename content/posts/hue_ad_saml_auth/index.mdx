...
* `emailAddress` returns the _NameID_ claim in e-mail address format
*  `persistent` returns the _NameID_ claim as a pairwise identifier, a randomly-generated value. This is created once and the same value is returned every subsequent time.
*  `transient` returns the _NameID_ claim as a randomly generated value. A new value is generated each time a user logs in.
