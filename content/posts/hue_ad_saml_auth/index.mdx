...
These SAML tokens contain _claims_ (ie. user name, email address etc.) which are used by Hue when each new user logs in for the first time.
...
Specically, Azure AD requires a `.pfx` filetype and `rsa` encryption while Hue requires `.PEM` files with a `X.509` certificate. 
