...
The [VancouvR](https://github.com/mountainMath/VancouvR) package (an R interface for this API) was a much-appreciated tool in quickly and easily pulling this data to my local workspace. 
...
My original curiosity was just to figure out the number of Starbucks locations in Downtown Vancouver. 
...
Within the city boundaries, there are many unique neighbourhoods. The City of Vancouver uses these neighbourhoods to categorize business licenses, so this is a convenient identifier that is already present within the data.
...
For example, Starbucks has premises registered under the business types of _'Wholesale  Dealer'_ and _'Instruction'_ at _2930 VIRTUAL WAY_ but these aren't businesses intended for the general public.
...
I opted to focus specifically on Downtown locations. This was primarily motivated by the volume of store locations and the need to limit the volume displayed in each chart.
...
Ultimately, I feel that the interactive maps below offer a simpler and more-elegant solution, but this style of plot was an interesting one to create. 
...
Given the scale of data in this project, I opted not to script further cleaning of the data, [I have outputted the raw data aggregations to GitHub](https://github.com/oisinBates/vancouver_coffee_businesses/tree/main/output/raw_data). 

Data cleanliness aside, a few interesting observations can be found:
  - All Starbucks stores in the City of Vancouver are corporate run except for the Wall Centre Starbucks.
...
[The full data aggregation from 2013-2023 is viewable on GitHub](https://github.com/oisinBates/vancouver_coffee_businesses/blob/main/output/raw_data/premises_and_employee_count.csv).

One obvious differentiation not present is full-time versus part-time employees. 
Also, it is of note that some stores focus primarily on coffee while McDonald's for example, though famous for its coffee, serves a much larger menu.

## Afterthoughts

My goal here was originally just to get some hard numbers to understand the declining presence of Starbucks in Vancouver. 
From here, the focus specifically on coffee chains was a logical comparison, in part due to the volume of data available and the level of data cleaning that would be necessary to properly analyze all of Vancouver's smaller coffee stores.

Having said that, it would not be fitting to conclude without acknowledging some of the incredible specialty coffee available in Vancouver. 
